...
/**
 * Creates a split pane with the split being only 1px wide, whilst the clickable
 * area is larger, allowing for easy use.
 */
public class JMinimalSplitPane extends JSplitPane {
    ...
    /**
     * Full constructor which takes an orientation as well as the two
     * components to use - one for each side of the split.
     *
    ...
    /**
     * Where we do our hack to reduce the draw size of the split, but
     * increase the clickable area of the split handle to resize the split.
     */
    @Override
    ...
    /**
     * Gets the size offset (i.e. how much of the drag size is on either side
     * of the divider).
     *
    ...
    /**
     * Allows the divider's invisible click area to be biased towards one side
     * or the other of the split.
     *
     * @param dividerDragOffset Amount of pixels to offset from being completely
    ...
    private class JMinimalSplitPaneUI extends BasicSplitPaneUI {
        /**
         * Callback which sets our small divider as the divider for the split
         * pane.
         *
    ...
    /**
     * Divider which always draws at 1px thickness, but handles click events
     * for its entire invisible area.
     */
    private class ZeroSizeDivider extends BasicSplitPaneDivider {
        ...
        /**
         * Finalise the drag, using the super method, but accounting for the
         * invisible drag area's offset.
         *
